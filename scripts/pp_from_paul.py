...
class pure_pursuit:
    ...
    def __init__(self, racecar_name, waypoint_fil, publication_topic=None):
        ...
        self.point_in_car_frame = rospy.Publisher('/' + racecar_name + '/goal_point_car_frame', MarkerArray,
                                                  queue_size=5)
        # Subscriber to vehicle position
        rospy.Subscriber(racecar_name + "/gt_pose", PoseStamped, self.callback, queue_size=1)
    ...
    def read_waypoints(self):
        ...
        # list of xy pts
        self.xy_points = np.hstack((self.path_points_x.reshape((-1, 1)), self.path_points_y.reshape((-1, 1)))).astype(
            'double')
